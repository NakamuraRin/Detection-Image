...
def main():
    ...
    # タイトルの表示
    st.title("コスメ判定アプリ")
    st.caption('「アイシャドウ」「リップ」「パウダー」のうち、どれに当たるかを判定してくれます。') 
    ...
    if uploaded_file is not None:
        ...
        pred0 = round(prediction[0],3) * 100  # アイシャドウの確率(%)
        pred1 = round(prediction[1],3) * 100  # リップの確率(%)
        pred2 = round(prediction[2],3) * 100  # パウダーの確率(%)

        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「アイシャドウ」です！")
        elif classNo == 1 :
            st.subheader(f"これは{pred1}％の確率で「リップ」です！")
        elif classNo == 2 :
            st.subheader(f"これは{pred2}％の確率で「パウダー」です！")
